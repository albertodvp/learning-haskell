...
isVisible trees i = or $ and . map (compareVisible trees i) <$> getTargetIndexes i trees
...
p2 :: Trees -> Int
p2 trees = maximum $ map (scenicScore trees) [(i,j) | i <- [1..M.nrows trees], j <- [1..M.ncols trees]]
  where
